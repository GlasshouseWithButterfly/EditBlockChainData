...
class UserController extends Controller
{
    ...
    public function fileExport($batch, $process)
    {
        ...
        // dd($rename_to);
        return Excel::download(new FileExport($batch), $rename_to);
    }
    ...
    public function process_one($batch)
    {
        ...
        $data = demotable::whereIn('From_Address', $address_csv);
        $data->whereNull('Status')->update(['Status' => 'Transfer']);
        $data = demotable::whereIn('To_Address', $address_csv);
        $data->whereNull('Status')->update(['Status' => 'Transfer']);
    ...
    public function process_three($batch)
    {
        $obj = new demotable;
    ...
    {
        $demotable = demotable::truncate();
        $batch = batch::truncate();
